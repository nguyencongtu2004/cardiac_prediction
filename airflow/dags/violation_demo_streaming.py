...
# Duration in seconds
STREAM_DURATION = int(os.getenv('STREAM_DURATION', '600'))  # 10 minutes default
...
# Task 1.5: Reset Kafka Topics (clear old messages)
reset_kafka_topics = BashOperator(
    ...
    TOPICS="helmet_video_frames helmet_violations redlight_video_frames redlight_violations"
    
    for TOPIC in $TOPICS; do
        echo "[1/2] Deleting topic: $TOPIC"
        kafka-topics --bootstrap-server $KAFKA_BOOTSTRAP --delete --topic $TOPIC 2>/dev/null || true
        sleep 1
        
        echo "[2/2] Creating topic: $TOPIC"
        kafka-topics --bootstrap-server $KAFKA_BOOTSTRAP --create --topic $TOPIC --partitions 1 --replication-factor 1 2>/dev/null || true
    done
...
# Init → Reset Kafka → [Producer + Helmet + RedLight] (tất cả song song) → Summary
init_check >> reset_kafka_topics >> [start_producer, helmet_detector, redlight_detector] >> summary
