...
3.  **Stream Processing**: **Spark Structured Streaming** consumes data from Kafka, applies a pre-trained **Ensemble ML Model** (Random Forest + Logistic Regression + GBT with SMOTE balancing), and writes predictions to PostgreSQL.
...
1.  Train ensemble models (Random Forest + Logistic Regression + GBT) with SMOTE balancing on the latest data.
2.  Evaluate each model individually and compare ensemble performance.
3.  Automatically promote the best performing model to production.

### 🤖 Machine Learning Models

The system uses an **ensemble approach** inspired by best practices:

- **SMOTE Balancing**: Handles class imbalance in training data
- **Random Forest**: Main production model (numTrees=100, maxDepth=25)
- **Logistic Regression**: L2 regularization for linear patterns
- **Gradient Boosting Trees**: Captures non-linear interactions
- **Hard Voting Ensemble**: Weighted voting (RF:2, LR:2, GBT:1) for final predictions

Model performance is continuously monitored, with metrics stored for each training run.
